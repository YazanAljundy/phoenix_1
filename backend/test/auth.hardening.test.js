// The hardening package from the C-1/H-*/M-1 audit round, driven over real HTTP
// against the real Express app so the routes, limiters, controllers and services
// are all in the picture - the same pattern as auth.register.test.js.
//
// Sits alongside auth.security.test.js, which covers the separate F-01..F-09
// round. The two rounds were written in parallel without seeing each other and
// were reconciled in one merge; the suites stayed separate because they assert
// different findings, not different views of the same ones.
//
// Covers:
...
test('admin reset: recovers a locked-out account and clears the lockout', async () => {
  ...
  const res = await call('POST', `/auth/admin/reset-password/${stored._id}`, {
    token,
    body: { newPassword: newPassword, reason: 'Owner called support, verified by licence number.' },
  });
...
test('admin reset: writes an audit record naming the admin, with no password material', async () => {
  ...
  const res = await call('POST', `/auth/admin/reset-password/${stored._id}`, {
    token,
    body: { newPassword: 'audited-new-password', reason: 'Verified in person.' },
  });
...
test('admin reset: enforces the TARGET role minimum, not the acting admin one', async () => {
  ...
  const tooShort = await call('POST', `/auth/admin/reset-password/${stored._id}`, {
    token,
    body: { newPassword: 'short7c' },
  });
  ...
  const nineChars = await call('POST', `/auth/admin/reset-password/${warehouseUser._id}`, {
    token,
    body: { newPassword: 'nine-char' },
  });
...
test('admin reset: refused to non-admins, and cannot target another admin', async () => {
  ...
  // No token at all.
  const anon = await call('POST', `/auth/admin/reset-password/${stored._id}`, {
    body: { newPassword: 'anonymous-attempt' },
  });
  ...
  const asPharmacy = await call('POST', `/auth/admin/reset-password/${stored._id}`, {
    token: login.body.token,
    body: { newPassword: 'self-service-attempt' },
  });
  ...
  const adminOnAdmin = await call('POST', `/auth/admin/reset-password/${otherAdmin._id}`, {
    token,
    body: { newPassword: 'lateral-movement-1234' },
  });
  // The unified endpoint answers this explicitly rather than pretending the
  // account does not exist: the caller is already an admin and can list every
  // account anyway, so there is nothing to hide from them - only something to
  // refuse.
  assert.strictEqual(adminOnAdmin.status, 403);
  assert.strictEqual(adminOnAdmin.body.code, 'ADMIN_ACCOUNT_NOT_RESETTABLE');
});
...
test('admin reset: refuses a deleted account', async () => {
  ...
  const res = await call('POST', `/auth/admin/reset-password/${stored._id}`, {
    token,
    body: { newPassword: 'reviving-the-dead' },
  });
  assert.strictEqual(res.status, 400);
